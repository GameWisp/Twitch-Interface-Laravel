 ...
 *
 * We re-wrote the hell out of this.
 */
...
class TwitchInterface extends CustomApiInterface
{
  protected $twitch_clientKey = '';
  protected $twitch_clientSecret = '';
  protected $this->twitch_clientUrl = '';
  ...
  public function __construct($params = array()) {
    ...
    $this->twitch_debugLevels = Config::get("twitch-interface-laravel.twitch_debug_levels");
    
    // This holds many of the limitation settings for performing calls, stored as an array for ease of calling [Keyed]
    $this->twitch_configuration = Config::get("twitch-interface-laravel.twitch_configuration");
    
    // Generate some warnings/errors if needed variables are not set.
    if ($this->twitch_clientKey == "") {
      Log::error("Warning: Client key is not defined.  This won't work.");
    } 
    else {
      
      //Log::info("Client key: '$this->twitch_clientKey'");
      
    }
    if ($this->twitch_clientSecret == "") {
      Log::error("Warning: Client Secret key is not defined.  This won't work.");
    } 
    else {
    ...
    if ($this->twitch_clientUrl == "") {
      Log::error("Warning: Twitch Client Url is not defined.  This won't work.");
    } 
    else {
      
      //Log::info("Twitch Client Url host: '$this->twitch_clientUrl'");
      
    }
    
    $this->twitch_configuration["DEBUG_SUPPRESSION_LEVEL"] = $this->twitch_debugLevels['ALL'];
  }
  ...
  protected function generateOutput($function, $errStr, $outputLevel = 4) {
    ...
    if ($this->twitch_configuration['DEBUG_SUPPRESSION_LEVEL'] >= $outputLevel) {
      
      // Enter your output format code here
      Log::info("Function Name: " . $function . " | Error: " . $errStr);
    }
  }
  ...
  protected function cURL_get($url, array $get = array() , array $options = array() , $returnStatus = false) {
    ...
    // Specify the header
    $header = array(
      'Accept: application/vnd.twitchtv.v' . $this->twitch_configuration['API_VERSION'] . '+json'
    );
     // Always included
    $header = (($this->twitch_configuration['TOKEN_SEND_METHOD'] == 'HEADER') && ((array_key_exists('oauth_token', $get) === 1) || (array_key_exists('oauth_token', $get) === true))) ? array_merge($header, array(
      'Authorization: OAuth ' . $get['oauth_token']
    )) : $header;
    $header = (($this->twitch_clientKey !== '') && ($this->twitch_clientKey !== ' ')) ? array_merge($header, array(
      'Client-ID: ' . $this->twitch_clientKey
    )) : $header;
    
    if (($this->twitch_configuration['TOKEN_SEND_METHOD'] == 'HEADER') && ((array_key_exists('oauth_token', $get) === 1) || (array_key_exists('oauth_token', $get) === true))) {
    ...
    $this->generateOutput($functionName, 'API Version set to: ' . $this->twitch_configuration['API_VERSION'], 3);
    ...
    $default = array(
      ...
      CURLOPT_CONNECTTIMEOUT => $this->twitch_configuration['DEFAULT_RETURN_TIMEOUT'],
      CURLOPT_TIMEOUT => $this->twitch_configuration['DEFAULT_TIMEOUT'],
      CURLOPT_HTTPHEADER => $header
    );
    
    // Do we have a certificate to use?  if OpenSSL is available, there will be a certificate
    if ($this->twitch_configuration['CERT_PATH'] != '') {
      ...
      array_merge($default, array(
        ...
        CURLOPT_CAINFO => realpath($this->twitch_configuration['CERT_PATH'])
         // This requires the real path of the certificate (Strict, may use CAPATH instead if it causes problems)
  ...
  protected function cURL_post($url, array $post = array() , array $options = array() , $returnStatus = false) {
    ...
    $header = array(
     ...
     // Always included
    $header = (($this->twitch_configuration['TOKEN_SEND_METHOD'] == 'HEADER') && ((array_key_exists('oauth_token', $post) === 1) || (array_key_exists('oauth_token', $post) === true))) ? array_merge($header, array(
    ...
    if (($this->twitch_configuration['TOKEN_SEND_METHOD'] == 'HEADER') && ((array_key_exists('oauth_token', $post) === 1) || (array_key_exists('oauth_token', $post) === true))) {
  ...
  protected function cURL_put($url, array $put = array() , array $options = array() , $returnStatus = false) {
    ...
    $header = array(
     ...
     // Always included
    $header = (($this->twitch_configuration['TOKEN_SEND_METHOD'] == 'HEADER') && ((array_key_exists('oauth_token', $put) === 1) || (array_key_exists('oauth_token', $put) === true))) ? array_merge($header, array(
    ...
    if (($this->twitch_configuration['TOKEN_SEND_METHOD'] == 'HEADER') && ((array_key_exists('oauth_token', $put) === 1) || (array_key_exists('oauth_token', $put) === true))) {
  ...
  private function get_iterated($functionName, $url, $options, $limit, $offset, $arrayKey = null, $authKey = null, $hls = null, $direction = null, $channels = null, $embedable = null, $client_id = null, $broadcasts = null, $period = null, $game = null, $returnTotal = false, $sortBy = null) {
    ...
    if ($limit == - 1) {
      $toDo = 100000000;
       // Set to an arbritrarily large number so that we can itterate forever if need be
      
    } 
    else {
      $toDo = $limit;
       // We have a finite amount of iterations to do, account for the _links object in the first return
      
    }
    
    // Calculate the starting limit
    if ($toDo > ($this->twitch_configuration[$this->twitch_configuration['CALL_LIMIT_SETTING']] + 1)) {
      $startingLimit = $this->twitch_configuration[$this->twitch_configuration['CALL_LIMIT_SETTING']];
    ...
    if ($return == 0) {
      for ($i = 1; $i <= $this->twitch_configuration['RETRY_COUNTER']; $i++) {
        $return = json_decode($this->cURL_get($url, $get, $options) , true);
    ...
    while ((($toDo > ($this->twitch_configuration[$this->twitch_configuration['CALL_LIMIT_SETTING']] + 1)) && ($toDo > 0)) || ($limit == - 1)) {
      ...
      if ($toDo == 1) {
        $toDo = 2;
         // Catch this, it will drop one return
      ...
      if (($toDo > $startingLimit) && ($toDo > 0) && ($limit != - 1)) {
        ...
        $get = array(
          'limit' => $currentReturns + $startingLimit,
  ...
  public function generateToken($code) {
    ...
    $post = array(
      'client_id' => $this->twitch_clientKey,
      'client_secret' => $this->twitch_clientSecret,
      'grant_type' => 'authorization_code',
      'redirect_uri' => $this->twitch_clientUrl,
      'code' => $code
    );
    $options = array();
  ...
  public function generateAuthorizationURL($grantType) {
    ...
    $urlRedirect = 'https://api.twitch.tv/kraken/oauth2/authorize?' . 'response_type=code' . '&' . 'client_id=' . $this->twitch_clientKey . '&' . 'redirect_uri=' . $this->twitch_clientUrl . '&' . 'scope=' . $scopes;
  ...
  public function retrieveRedirectCode($url) {
    ...
    $code = $this->getURLParamValue($url, 'code');
  ...
  public function getBlockedUsers($chan, $limit = - 1, $offset = 0, $authKey, $code, $returnTotal = false) {
    ...
    if (($authKey != null || '') || ($code != null || false)) {
      if ($authKey != null || '') {
        ...
        if ($check["token"] != false) {
          ...
          if ($code != null || '') {
            $auth = $this->generateToken($code);
             // Assume generation and check later for failure
            
          } 
          else {
            $this->generateError(400, 'Existing token expired and no code available for generation.');
            return array();
             // return out here, match the fail state of the call
       ...
       // check the token value
      {
        ...
        return array();
         // return out after the error is passed, match the fail tate of the call
      ...
      if (!$authSuccessful) {
        ...
        return array();
         // Match the fail state of the call so users are not thrown off
    ...
    $options = array();
     // For things where I don't put in any default data, I will leave the end user the capability to configure here
    $usernames = array();
    ...
    foreach ($usernamesObject as $key => $user) {
      ...
      $this->generateOutput($functionName, 'Setting Key for username: ' . $user['user'][$this->twitch_configuration['KEY_NAME']], 3);
      $usernames[$counter] = $user['user'][$this->twitch_configuration['KEY_NAME']];
      $counter++;
    }
  ...
  public function addBlockedUser($chan, $username, $authKey, $code) {
    ...
    if (($authKey != null || '') || ($code != null || false)) {
       ...
       // check the token value
      {
        ...
        return false;
         // return out after the error is passed, match the fail state of the call
  ...
  public function getChannelObject($chan) {
    ...
    if (!is_array($object)) {
      $object = array();
       // Catch to make sure that an array is returned no matter what, technically our fail state
  ...
  public function getChannelObject_Authd($authKey, $code) {
    ...
    if (($authKey != null || '') || ($code != null || false)) {
       ...
       // check the token value
      {
        ...
        return array();
         // return out after the error is passed
  ...
  public function getEditors($chan, $limit = - 1, $offset = 0, $authKey, $code, $returnTotal = false) {
    ...
    foreach ($editorsObject as $key => $editor) {
      ...
      $editors[$counter] = $editor[$this->twitch_configuration["KEY_NAME"]];
    }
  ...
  public function getFollowers($chan, $limit = - 1, $offset = 0, $sorting = 'desc', $returnTotal = false) {
    ...
    foreach ($followersObject as $k => $follower) {
      ...
      $key = $follower['user'][$this->twitch_configuration['KEY_NAME']];
  ...
  public function getFollows($username, $limit = - 1, $offset = 0, $sorting = 'desc', $sortBy = 'created_at', $returnTotal = false) {
    ...
    foreach ($channelsObject as $k => $channel) {
      ...
      $key = $channel['channel'][$this->twitch_configuration['KEY_NAME']];
  ...
  public function getStreamsObjects($game = null, $channels = array() , $limit = - 1, $offset = 0, $embedable = false, $hls = false, $client_id = null, $returnTotal = false) {
    ...
    foreach ($streamsObject as $key => $value) {
      ...
      foreach ($value as $k => $v) {
        if ($k == 'channel') {
          $objKey = $v[$this->twitch_configuration['KEY_NAME']];
  ...
  public function getFeaturedStreams($limit = - 1, $offset = 0, $hls = false, $returnTotal = false) {
    ...
    foreach ($featuredObject as $key => $value) {
      ...
      if (($key != 'self') && ($key != 'next')) {
        $k = $value['stream']['channel'][$this->twitch_configuration['KEY_NAME']];
        $featured[$k] = $value;
      }
    }
  ...
  public function getFollowedStreams($limit = - 1, $offset = 0, $authKey, $code, $hls = false, $returnTotal = false) {
    ...
    foreach ($streamsObject as $key => $value) {
      ...
      if (($key != 'self') && ($key != 'next')) {
        $k = $value['channel'][$this->twitch_configuration['KEY_NAME']];
        $this->generateOutput($functionName, 'Setting key: ' . $k, 3);
        $streams[$k] = $value;
      }
    }
  ...
  public function getChannelSubscribers($chan, $limit = - 1, $offset = 0, $direction = 'asc', $authKey, $code, $returnTotal = false) {
    ...
    foreach ($subscribersObject as $k => $subscriber) {
      ...
      $key = $subscriber['user'][$this->twitch_configuration['KEY_NAME']];
  ...
  public function getTeams($limit = - 1, $offset = 0, $returnTotal = false) {
    ...
    foreach ($teamsObject as $k => $team) {
      ...
      $key = $team[$this->twitch_configuration['KEY_NAME']];
      $teams[$key] = $team;
      $this->generateOutput($functionName, 'Setting key: ' . $key, 3);
    }
